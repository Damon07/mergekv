...
string ByteBufferReader::Path() const {
    return path_;
}

size_t ByteBufferReader::Read(bytes &p) {
    size_t n = std::min(p.size(), data_->size() - read_pos_);
    std::copy(data_->begin() + read_pos_, data_->begin() + read_pos_ + n, p.begin());
    read_pos_ += n;
    return n;
}
...
void ByteBuffer::MustReadAt(bytes &p, size_t offset) {
    if (data_->size() < offset+p.size()) {
        throw InvalidInputException("BUG: too big offset=%d; can not exceed size=%d", offset, data_->size());
    }
