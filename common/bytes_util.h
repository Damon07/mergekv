...
ByteBufferReader(std::shared_ptr<bytes> data,const string& path): data_(data), read_pos_(0), path_(path) {};
...
string Path() const override { 
    return fmt::format("ByteBuffer/{:x}/mem", reinterpret_cast<std::uintptr_t>(this));
}
