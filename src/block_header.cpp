...
bytes_const_span BlockHeader::UnmarshalNoCopy(bytes_const_span src) {
  ...
  if (items_block_size > 2 * kMaxInmemoryBlockSize) {
    throw InvalidInputException(
        "too big itemsBlockSize; got %d; cannot exceed %d", items_block_size,
        2 * kMaxInmemoryBlockSize);
  }

  if (lens_block_size > 2 * 8 * kMaxInmemoryBlockSize) {
    throw InvalidInputException(
        "too big lensBlockSize; got %d; cannot exceed %d", lens_block_size,
        2 * 8 * kMaxInmemoryBlockSize);
  }
...
//
// It is expected that src remains unchanged while rhe returned blocks are in
// use.
void UnmarshalBHNoCopy(std::vector<BlockHeader> &dst, bytes_const_span src,
                       int bh_count) {
  if (bh_count <= 0) {
  ...
  if (src.size() != 0) {
    throw InvalidInputException("unexpected non-zero tail left after "
                                "unmarshaling %d block headers; len(tail)=%d",
                                bh_count, src.size());
  }

  auto new_bhs = std::span<BlockHeader>(dst.data() + dst_len, bh_count);
  if (!std::is_sorted(new_bhs.begin(), new_bhs.end(),
                      [](const BlockHeader &a, const BlockHeader &b) {
